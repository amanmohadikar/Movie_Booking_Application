...
//     if (!isEmailPresent) { return res.status(404).json({ message: "Email not Exist" }); }
...
//     let isIdExist = await User.findById(id)
...
//     let updatedUser = await User.findOneAndUpdate(id, { $set: { name: name, email: email } })
...
//     let bookings = await Bookings.find({ user: id }).populate("movie").populate("user");
